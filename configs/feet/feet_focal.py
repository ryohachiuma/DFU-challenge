...
log_config = dict(
    ...
    hooks=[
        ...
        dict(type='WandbLoggerHook', init_kwargs=dict(project='mmdetection-tools', name='feet_focal'))
    ])
...
test_cfg = dict(
    rcnn=dict(
        ...
        max_per_img=100))
model = dict(rpn_head=dict(loss_cls=dict(
            type='FocalLoss',
            use_sigmoid=True,
            gamma=2.0,
            alpha=0.25,
            loss_weight=1.0)),roi_head=dict(bbox_head=dict(num_classes=1)))
