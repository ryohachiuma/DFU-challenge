...
log_config = dict(
    ...
    hooks=[
        ...
        dict(type='WandbLoggerHook', init_kwargs=dict(project='mmdetection-tools', name='feet_anchor'))
    ])
...
test_cfg = dict(
    rcnn=dict(
        ...
        max_per_img=100))
model = dict(roi_head=dict(bbox_head=dict(num_classes=1)), rpn_head=dict(anchor_generator=dict(strides=[3, 6, 10, 17, 34])))
